...
<div class="px-4 md:px-0 space-y-6 md:space-y-8" x-data="{ showModal: false }">
    ...
    <div class="bg-white p-6 md:p-8 rounded-3xl md:rounded-[40px] shadow-sm border border-teal-50 flex flex-col md:flex-row items-center gap-6">
        <div class="w-20 h-20 bg-teal-500 rounded-[2rem] flex items-center justify-center text-white text-3xl shadow-xl shadow-teal-100 border-4 border-white shrink-0">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
        <!-- Akun Orang Tua -->
        <div class="bg-white p-6 md:p-8 rounded-3xl shadow-sm border border-teal-50 space-y-6">
            <h4 class="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                <i class="fas fa-id-badge text-teal-500"></i> Detail Akun
            </h4>
            <div class="space-y-4">
        ...
        <div class="md:col-span-2 bg-white p-6 md:p-8 rounded-3xl shadow-sm border border-teal-50 space-y-6">
            <h4 class="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                <i class="fas fa-graduation-cap text-teal-500"></i> Informasi Siswa Terkait
            </h4>
    ...
    <div x-show="showModal" x-cloak class="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-slate-900/60 backdrop-blur-sm">
        <div class="bg-white rounded-[40px] p-8 w-full max-w-md shadow-2xl relative" @click.away="showModal = false">
            <div class="text-center mb-8">
                <div class="w-16 h-16 bg-teal-100 text-teal-500 rounded-2xl flex items-center justify-center mx-auto mb-4 text-xl">
            ...
            <form action="{{ route('ortu.password.update') }}" method="POST" class="space-y-4">
                ...
                <div class="flex gap-3 pt-6">
                    ...
                    <button type="submit" class="flex-1 py-4 bg-teal-500 text-white font-bold rounded-2xl text-[10px] uppercase tracking-widest shadow-lg shadow-teal-100">Simpan Perubahan</button>
                </div>
            </form>
        </div>
    </div>
